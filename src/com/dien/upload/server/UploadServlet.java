...
import static com.dien.upload.shared.UConsts.PARAM_DELAY;
import static com.dien.upload.shared.UConsts.TAG_BLOBSTORE;
import static com.dien.upload.shared.UConsts.TAG_BLOBSTORE_PATH;
import static com.dien.upload.shared.UConsts.TAG_CANCELED;
import static com.dien.upload.shared.UConsts.TAG_ERROR;
import static com.dien.upload.shared.UConsts.TAG_FINISHED;
...
import com.dien.upload.server.exceptions.UploadCanceledException;
import com.dien.upload.server.exceptions.UploadException;
import com.dien.upload.server.exceptions.UploadSizeLimitException;
import com.dien.upload.server.exceptions.UploadTimeoutException;
import com.dien.upload.shared.UConsts;
 ...
 *     &lt;servlet-class&gt;com.dien.upload.server.UploadServlet&lt;/servlet-class&gt;
 *   &lt;/servlet&gt;
 *   
 *   &lt;servlet-mapping&gt;
 *     &lt;servlet-name&gt;uploadServlet&lt;/servlet-name&gt;
...
public class UploadServlet extends HttpServlet implements Servlet {
  ...
  protected static final String XML_CANCELED_TRUE = "{ret:true,type:'canceled',msg:'取消成功'}";
  protected static final String XML_DELETED_TRUE = "{ret:true,type:'delete',msg:'删除成功'}";
  protected static final String XML_ERROR_ITEM_NOT_FOUND = "{ret:false,type:'not_found',msg:'item not found'}";
  protected static final String XML_ERROR_TIMEOUT = "{ret:false,type:'timeout',msg:'timeout receiving file'}" ;
  protected static final String XML_FINISHED_OK = "{ret:false,type:'finished',msg:'ok'}";
  ...
  protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
    ...
    if (request.getParameter(UConsts.PARAM_SESSION) != null) {
      ...
      request.getSession();
      renderJSONResponse(request, response, "{ret:true,"+TAG_BLOBSTORE+":" + (isAppEngine() && useBlobstore) + "}" );
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
    ...
    try {
      ...
      renderXmlResponse(request, response, "{ret:false,msg='" + e.getMessage() + "'}", true);
    } catch (Exception e) {
      logger.error("UPLOAD-SERVLET (" + request.getSession().getId() + ") Exception -> " + e.getMessage() + "\n" + stackTraceToString(e));
      error = e.getMessage();
      renderXmlResponse(request, response, "{ret:false,msg='" + error + "'}", true);
    } finally {
      perThreadRequest.set(null);
    }
  }

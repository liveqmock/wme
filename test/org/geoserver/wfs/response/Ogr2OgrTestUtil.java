...
public class Ogr2OgrTestUtil {
    private static Logger LOGGER = Logger.getLogger(Ogr2OgrTestUtil.class);
    ...
    public static boolean isOgrAvailable() {
        ...
        if (IS_OGR_AVAILABLE == null) {
            try {
                ...
                SDEWrapper ogr = new SDEWrapper(OGR2OGR, GDAL_DATA);
                IS_OGR_AVAILABLE = ogr.isAvailable();
            } catch (Exception e) {
                IS_OGR_AVAILABLE = false;
                e.printStackTrace();
                LOGGER.error("Disabling ogr2ogr output format tests, as ogr2ogr lookup failed", e);
            }
        }
